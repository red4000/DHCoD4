...
int __cdecl xde(unsigned char *opcode)
{
   struct xde_instr i;

   return(xde_disasm(opcode, &i));
}
                                ...
                                /* OUT */ struct xde_instr *diza)
{
   ...
   if (flag & C_MODRM)
   {
      ...
      if (c == 0xF6)
      {
         flag |= xde_table[TBL_F6 + reg];
      }
      if (c == 0xF7)
      {
         flag |= xde_table[TBL_F7 + reg];
      }
      if (c == 0xFE)
      {
         flag |= xde_table[TBL_FE + reg];
      }
      if (c == 0xFF)
      {
         flag |= xde_table[TBL_FF + reg];
      }
